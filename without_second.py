...
str_d1 = '2023/08/31 , 19:30'
str_d2 = '2023/08/28 , 01:00'
...
delta = d1 - d2
dt_store = np.char.array(np.zeros((5430, 3)), unicode=True)
i = 0
pics = []
while delta.days > 0 or delta.seconds//3600 > 0 or \
      delta.seconds % 3600//60 > 0:
    delta -= timedelta(days=0, seconds=0, microseconds=0,
                       milliseconds=0, minutes=1, hours=0, weeks=0)
...
for i in range(60*48, 60*60):
    ...
    drawb.text((225, 170), dt_store[i, 1][0], anchor="mm",
               fill=(255, 255, 255, 0), font=fontN)
    drawb.text((412.5, 170), dt_store[i, 1][1], anchor="mm",
               fill=(255, 255, 255, 0), font=fontN)
    drawb.text((512.5, 170), dt_store[i, 2][0], anchor="mm",
               fill=(255, 255, 255, 0), font=fontN)
    drawb.text((557.5, 170), dt_store[i, 2][1], anchor="mm",
               fill=(255, 255, 255, 0), font=fontN)
    ...
    # ImageShow.show(out)
    print(round(i/(60*12), 4)*100)
pics[0].save("/home/hamze/personal/out6.gif", save_all=True,
             optimize=True,
             append_images=pics[1:], duration=1000*60, loop=0)
